...
import json
API_ID = '24414297'
API_HASH = '4c17d07e937e051108373a89b6973cff'
bot_token = ""
running_processes = {}
try:
    ...
    if update.message and update.message.chat.type == "private":
        if (str(update.message.chat.id) == str(info["sudo"])):
            ...
            keyboard = [
                ...
                [
                    InlineKeyboardButton(
                        "انضمام قناه ", callback_data="addecho"),
                ],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await update.message.reply_text(""" ╭──⌯༺Source Tethon༻⌯──╮
※ 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 -  ‌‌    ※\n
※ 𝗩𝗘𝗥𝗦𝗜𝗢𝗡 - 𝟭.𝟭 - 𝗥𝗘𝗩𝗜𝗦𝗘𝗗   ※\n
※ 𝗗𝗘𝗩𝗘𝗟𝗢𝗣𝗘𝗥 - @W_Q_3h  \n
╰───⌯༺Source Tethon༻ ⌯───╯\n سرعه تجميع هي :""" + str(info["sleeptime"]), reply_markup=reply_markup)
...
╰───⌯༺Source Tethon༻ ⌯───╯\nسرعه تجميع الخاصه بك هي : """  + str(info["sleeptime"]), reply_markup=reply_markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (query.data == "addecho"):
        ...
        if (str(query.message.chat.id) == str(info["sudo"])):
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(""" ╭──⌯༺Source Tethon༻⌯──╮
...
╰───⌯༺Source Tethon༻ ⌯───╯\n سرعه تجميع هي : """ + str(info["sleeptime"]), reply_markup=reply_markup)
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("معتز عمك ®©\n سرعه تجميع هي : " + str(info["sleeptime"]), reply_markup=reply_markup)
